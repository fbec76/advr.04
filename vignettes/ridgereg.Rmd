...
We use the BostonHousing dataset from the `mlbench` package and split it 80/20 into training and test sets using `caret::createDataPartition()`.
...
Use caret’s `lmStepAIC` (AIC-based stepwise selection, since `leapForward` does not support intercept model) with `direction = "forward"`,
...
# Summary of results
On the test set, the full linear and forward-selection models perform identically (RMSE ≈ 4.589, \(R^2\) ≈ 0.761, MAE ≈ 3.365),
implying forward selection chose essentially the same predictors.
Ridge is marginally better on all metrics (RMSE 4.587, \(R^2\) 0.762, MAE 3.363), likely from shrinkage stabilizing coefficients.
These gains are tiny and likely within sampling noise, but show ridge can match or slightly exceed OLS.